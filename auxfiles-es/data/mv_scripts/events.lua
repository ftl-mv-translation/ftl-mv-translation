...
script.on_game_event("ANOMALY_ORACLE_SPEAK", false, function()
    Hyperspace.setWindowTitle("-Lee el mensaje de error :)")  --Here we set the title of the window, and then quickly reset it. Why is that? This is because when an error message pops up from Hyperspace.ErrorMessage, nothing is run until after the message is closed. This way, we only tell the player to "Check the error message" for as long as the message is open.
    Hyperspace.ErrorMessage("No confíes en el mercader, ni en sus cómplices. Confía solo en mí y en Los Observadores, informa a Thest sobre esto. Ellos te ayudarán. Encuéntrame. Y lo que sea que hagas, es de gran importancia que no le cuentes al del traje sobre esto. Discutiremos esto más profundo cuando nos encontremos.")
    reset_title()
    log(string.rep("Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame Encuéntrame\n", 99))
end)

script.on_game_event("SHES_MAD", false, function()
	Hyperspace.ErrorMessage("Uh, ¿disculpa? ¿Qué fue eso? ¡Teníamos un PLAN y no era esto!")
	Hyperspace.setWindowTitle(">:(")
end)

script.on_game_event("SHE_KILLED_YOU", false, function()
	Hyperspace.ErrorMessage("La única manera de la que podrías haber ganado era trabajando conmigo, idiota. ¡Hahe! ¡Hahe! ¡Hahe! ¡Hahe! ¡Hahe! ¡Hahe! ¡Hahe! ¡Hahe! ¡Hahe! ¡Hahe!")
...
do --Here we keep the variables "titleSet" and "herVirus" only where they are needed.
	...
	script.on_game_event("SHE_WINS", false, function()
		Hyperspace.ErrorMessage("¡Hahe! ¡Gané!")
		herVirus = true
		titleSet = false
	end)
...
script.on_game_event("HER_FINALE_REAL", false, function()
	Hyperspace.ErrorMessage("¡No puedo creer que me hayas traicionado así, Renegado! Estaba tan cerca... tan cerca de algo tan perfecto. Espero que estés feliz en tu Multiverso estéril y aburrido, viviendo bajo el régimen de una plagas de jardín tan comunes. >:(")
end)
