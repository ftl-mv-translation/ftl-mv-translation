...
script.on_game_event("ANOMALY_ORACLE_SPEAK", false, function()
    Hyperspace.setWindowTitle("エラーメッセージを確認してね :)")  --Here we set the title of the window, and then quickly reset it. Why is that? This is because when an error message pops up from Hyperspace.ErrorMessage, nothing is run until after the message is closed. This way, we only tell the player to "Check the error message" for as long as the message is open.
    Hyperspace.ErrorMessage("商人もその仲間も信用するな。私とオブザーバーだけを信じて、ゼストにこれを伝えて。彼らがあなたを助けてくれる。私を見つけて。そして何をするにしてもスーツの男にはこのことを内緒にすること。詳細は会ってからだ。")
    reset_title()
    log(string.rep("私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて私を見つけて\n", 99))
end)

script.on_game_event("SHES_MAD", false, function()
	Hyperspace.ErrorMessage("あの、すみません？今のは何？私たちには計画があったけど、これじゃない！")
	Hyperspace.setWindowTitle(">:(")
end)

script.on_game_event("SHE_KILLED_YOU", false, function()
	Hyperspace.ErrorMessage("愚かな君が勝つ唯一の方法は、私と組むことだ。アハ！アハ！アハ！アハ！アハ！アハ！アハ！アハ！アハ！アハ！")
	for i = 1, 5 do Hyperspace.ErrorMessage("アハ！アハ！アハ！アハ！アハ！アハ！アハ！アハ！アハ！アハ！") end --Here we repeat a function 5 times instead of writing it out multiple times. The word "for" can be used for iteration in this way. For example, "for i = 5, 1, -1 do print(i) end" will print "5 4 3 2 1".
...
do --Here we keep the variables "titleSet" and "herVirus" only where they are needed.
	...
	script.on_game_event("SHE_WINS", false, function()
		Hyperspace.ErrorMessage("アハ！私の勝ち！")
		herVirus = true
		titleSet = false
	end)
...
script.on_game_event("HER_FINALE_REAL", false, function()
	Hyperspace.ErrorMessage("こんなふうに私を裏切るなんて信じられない、レネゲイド！もう少しだったのに……もう少しで完璧だったのに。だったら幸せに暮らしなよ、無意味で退屈なマルチバースの中で、庭にいるつまんない、ぬめぬめした害虫に支配されながら。>:(")
end)
