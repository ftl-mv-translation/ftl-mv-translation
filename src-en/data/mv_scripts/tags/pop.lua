...
table.insert(weaponTagParsers, function(weaponNode)
    local popNode = weaponNode:first_node("mv-pop")
    if popNode then
        popWeapons[weaponNode:first_attribute("name"):value()] = {
            count = node_get_number_default(popNode:first_node("count"), 1),
            countSuper = node_get_number_default(popNode:first_node("countSuper"), 0)
        }
    end
end)
...
script.on_internal_event(Defines.InternalEvents.WEAPON_STATBOX, function(bp, stats)
    local popData = popWeapons[bp.name]
    if popData then
        stats = stats.."\n\n"..string_replace(Hyperspace.Text:GetText("stat_pop"), "\\1", popData.count)
        if popData.countSuper then
            stats = stats.."\n"..string_replace(Hyperspace.Text:GetText("stat_pop_super"), "\\1", popData.countSuper)
        end
